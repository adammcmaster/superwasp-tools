...
DATA_LOCATION = os.path.join(
    os.path.expanduser("~"), "Documents", "Archive", "PhD", "superwasp-data"
)
...
class PandasDFWrapper(object):
    ...
    def cached_pandas_load(self, filename):
        cache_file_path = pathlib.Path(
            os.path.join(CACHE_LOCATION, "{}.pickle".format(filename))
        )
        orig_file_path = pathlib.Path(os.path.join(DATA_LOCATION, filename))
        if cache_file_path.exists() and (
            not orig_file_path.exists()
            or cache_file_path.stat().st_mtime > orig_file_path.stat().st_mtime
        ):
...
class CoordinatesMixin(object):
    ...
    def coords(self):
        return SkyCoord(
            self.df["SWASP ID"].replace(r"^1SWASP", "", regex=True).values,
            unit=(u.hour, u.deg),
        )

    @property
    def fits_urls(self):
        return self.df["SWASP ID"].apply(
            lambda s: "http://wasp.warwick.ac.uk/lcextract?{}".format(
    ...
    def _coords_for_row(self, row, cache):
        if row["SWASP ID"] not in cache:
            cache[row["SWASP ID"]] = SkyCoord(
                row["SWASP ID"].replace("1SWASP", ""), unit=(u.hour, u.deg)
            )
    ...
    def add_vsx_types(self):
        ...
        if vsx_types is None:
            ...
            with shelve.open(os.path.join(CACHE_LOCATION, "vsx_cache")) as vsx_cache:
                with shelve.open(
                    os.path.join(CACHE_LOCATION, "coord_cache")
                ) as coord_cache:
                    for i, (_, row) in enumerate(self.df.iterrows(), start=1):
                        if i % 100 == 0:
                            print("Processing row: {}".format(i), end="\r")
                        vsx_query = self._query_vsx_for_coord(
                            self._coords_for_row(row, coord_cache), vsx_cache
                        )
                        if vsx_query is None:
                            continue

                        period_min = (row["Period"] / SECONDS_PER_DAY) * (
                            1 - self.VSX_PERIOD_THRESHOLD
                        )
                        period_max = (row["Period"] / SECONDS_PER_DAY) * (
                            1 + self.VSX_PERIOD_THRESHOLD
                        )
                        ...
                        for vsx_table in vsx_query:
                            for vsx_row in vsx_table:
                                ...
                                if vsx_row["f_min"] == self.VSX_MAG_AMPLITUDE_FLAG:
                                    if (
                                        vsx_row["max"] + vsx_row["min"]
                                    ) > self.VSX_MAGNITUDE_LIMIT:
                                        continue
                                else:
                                ...
                                for result_key, vsx_key in result_map.items():
                                    vsx_results_dict[result_key].append(
                                        vsx_row[vsx_key]
                                    )
...
class ZooniverseSubjects(PandasDFWrapper, ZooLookupMixin):
    def __init__(self, df=None):
        ...
        self.df, self.cache_file = self.cached_pandas_load(
            "superwasp-variable-stars-subjects.csv"
        )
    ...
    def subject_sets(self):
        return {
            set_id: self.get_subject_set(set_id)
            for set_id in set(self.df["subject_set_id"])
        }
    ...
    def workflows(self):
        return {
            ...
            for workflow_id in set(
                self.df[self.df["workflow_id"].notna()]["workflow_id"]
            )
        }
...
class ZooniverseClassifications(PandasDFWrapper):
    ...
    def __init__(
        ...
        duplicate_columns=("subject_ids", "user_id"),
    ):
        if df is not None:
            self.df = df
            return
        ...
        try:
            self.df, self.cache_file = self.cached_pandas_load(
                "superwasp-variable-stars-classifications.csv"
            )
            if self.df is not None:
                return
    ...
    def get_workflow(self, workflow_id):
        return ZooniverseClassifications(
            df=self.df[self.df["workflow_id"] == workflow_id]
        )

    def get_subjects(self, subject_ids):
        return ZooniverseClassifications(
            df=self.df[self.df["subject_ids"].isin(subject_ids)]
        )

    def get_users(self, user_names):
        return ZooniverseClassifications(
            df=self.df[self.df["user_name"].isin(user_names)]
        )
    ...
    def decode_annotations(self):
        ...
        self.df.drop(labels="annotations", axis="columns", inplace=True)
...
class FoldedLightcurves(PandasDFWrapper, CoordinatesMixin, ZooLookupMixin):
    def __init__(self, min_period=0, df=None):
        ...
        self.df = self.df[
            (self.df["Period Flag"] == 0) & (self.df["Period"] >= min_period)
        ]
        self.df["SWASP ID"] = self.df["SWASP"] + self.df["ID"]
        self.df.drop(
            ["Period Flag", "Camera Number", "SWASP", "ID"], "columns", inplace=True
        )
        self.df.to_pickle(self.cache_file)

    def get_siblings(self, swasp_id):
        return self.__class__(
            df=self.df[self.df["SWASP ID"] == swasp_id], min_period=self.min_period
        )
...
class AggregatedClassifications(PandasDFWrapper, CoordinatesMixin):
    ...
    def add_classification_labels(self):
        self.df = self.df.copy()
        self.df["Classification Label"] = self.get_classification_labels(
